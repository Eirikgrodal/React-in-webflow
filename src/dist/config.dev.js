...
exports.getPreviousWeekdays = getPreviousWeekdays;
exports.getNextWeekdays = getNextWeekdays;
exports.getCalenderDays = getCalenderDays;
...
function getPreviousWeekdays(isoDate) {
  var date = new Date(isoDate);
  console.log('iso ', isoDate);
  var dayOfWeek = getDayOfWeek(isoDate);
  var weekdays = [];
  ...
  return weekdays.sort(function (a, b) {
    return new Date(a.date).getTime() - new Date(b.date).getTime();
  });
  ; // array med dager i ISO format
}
...
function getNextWeekdays(isoDate) {
  var date = new Date(isoDate);
  var dayOfWeek = getDayOfWeek(isoDate);
  var weekdays = [];
...
function getCalenderDays(year, month, selectedDate) {
  var currentDays = getDaysInMonth(month, year, selectedDate);
  var previousDays = getPreviousWeekdays(currentDays[0].date);
  var nextDays = getNextWeekdays(currentDays.at(-1).date);
  return [].concat(_toConsumableArray(previousDays), _toConsumableArray(currentDays), _toConsumableArray(nextDays)); // array med dager i ISO format
}

var days = [// todo: make this automatic 
